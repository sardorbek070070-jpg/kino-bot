...
-------------------- Reklama yuborish (xabar ID sini qaytaradi) --------------------
async def send_ad(bot, chat_id: int, context: CallbackContext = None) -> int:
"""Reklamani yuboradi va yuborilgan xabar ID sini qaytaradi. Agar reklama bo'lmasa, None qaytaradi."""
ad = await get_ad()
if not ad:
return None
...
for key in ["main_msg_id", "ad_msg_id"]:
...
Reklama yuborish va ID sini saqlash
ad_msg_id = await send_ad(context.bot, chat_id, context)
if ad_msg_id:
context.user_data["ad_msg_id"] = ad_msg_id
...
Reklama yuborish va ID sini saqlash (agar kerak bo'lsa, bu yerda ham saqlash mumkin, lekin hozircha o'chirish shart emas)
await send_ad(context.bot, chat_id, context)
else:
